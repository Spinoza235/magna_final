...
const CalendrierPage: React.FC = () => {
  useEffect(() => {
    const tabs = document.querySelectorAll<HTMLButtonElement>('button[class*="px-4 py-1"]');

    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        tabs.forEach((t) => {
          t.classList.remove("bg-[#b8a898]", "text-white");
          t.classList.add("border", "border-amber-900", "text-[#864e00]");
        });
        tab.classList.add("bg-[#b8a898]", "text-white");
        tab.classList.remove("text-[#864e00]");
      });
    });
    ...
    const contactTanguBtn = allButtons.find((btn) =>
      btn.textContent?.includes("CONTACTER VOTRE TA-NGU")
    );
    const contactAdminBtn = allButtons.find((btn) =>
      btn.textContent?.includes("CONTACT ADMINISTRATEUR")
    );

    cancelBtn?.addEventListener("click", () => {
      if (window.confirm("Êtes-vous sûr de vouloir annuler les paiements ?")) {
        alert("Annulation des paiements en cours...");
      }
    });

    contactTanguBtn?.addEventListener("click", () => {
  ...
  return (
    <div className="bg-white min-h-screen font-sans overflow-y-auto relative p-3">
      {/* ✅ Fond montagne */}
      <div
        className="mountain-bg absolute inset-x-0 bottom-0 mx-auto w-full sm:w-4/5 md:w-3/5 h-full max-h-[600px] opacity-70 z-0"
        style={{
        background: "linear-gradient(to top, #8fbc8f 0%, #bef0be 40%, #b6f5c1 100%)",
        clipPath: "polygon(0% 100%, 8% 85%, 15% 75%, 22% 65%, 30% 55%, 38% 50%, 45% 45%, 52% 42%, 58% 45%, 65% 50%, 72% 55%, 78% 65%, 85% 75%, 92% 85%, 100% 100%)",
        height: "55vh",
        minHeight: "250px"
  }}
      ></div>
      ...
      <div className="relative z-10 text-center mt-3 w-full sm:w-4/5 md:w-3/5 mx-auto">
        ...
        <p className="text-sm italic text-[#864e00]">
          inscrite depuis 25 12 2024
        </p>
      </div>

      {/* ✅ Onglets */}
      <div className="relative z-10 flex justify-center space-x-2 mt-4 w-full sm:w-4/5 md:w-3/5 mx-auto">
        <button className="px-4 py-1 border border-amber-900 text-[#864e00] rounded-md text-sm">
          VOTRE PROFIL
        </button>
        <button className="px-4 py-1 border border-amber-900 text-[#864e00] rounded-md text-sm">
          VOTRE TA-NGU
        ...
        <button className="px-4 py-1 bg-[#b8a898] text-white border border-amber-900 rounded-md text-sm">
          LE CALENDRIER
        </button>
      </div>
        ...
        <div className="border border-[#c9a67b] rounded-md p-6">
          <div className="space-y-3 text-sm">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-[#472f00] font-medium">
                Evenements FONDATION KA
              </span>
            </div>
            <div className="flex items-center space-x-3">
          ...
          <div className="mt-6">
            ...
            <div className="grid grid-cols-7 gap-2 text-center text-[#472f00] text-sm font-medium">
              ...
              <div className="bg-red-600 text-white font-bold rounded-full w-7 h-7 mx-auto flex items-center justify-center">
                23
              </div>
              {[...Array(8)].map((_, i) => (
                <div key={i}>{24 + i}</div>
      ...
      <footer className="relative z-10 grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pb-10 text-center w-full sm:w-4/5 md:w-3/5 mx-auto">
        <button className="border-2 border-red-500 text-red-600 font-bold py-2 rounded-full text-sm">
          ANNULER LES PAIEMENTS
        </button>
        <button className="border border-amber-900 text-[#864e00] py-2 rounded-full text-sm">
          CONTACTER VOTRE TA-NGU
        </button>
        <button className="border border-amber-900 text-[#864e00] py-2 rounded-full text-sm">
